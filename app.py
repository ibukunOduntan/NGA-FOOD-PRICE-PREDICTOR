import streamlit as st 
 ...
                     # Ensure all selected states have data for the chosen food item 
                     states_with_data = df_compare['State'].unique() 
                     if len(states_with_data) < len(selected_states_to_compare): 
                         missing_states = set(selected_states_to_compare) - set(states_with_data) 
                         st.warning(f"No data available for {multi_state_food_item} in the following selected states: {', '.join(missing_states)}. They will not be shown on the chart.") 
                         df_compare = df_compare[df_compare['State'].isin(states_with_data)] 
                         if df_compare.empty: 
                             st.info(f"No sufficient data to compare for {multi_state_food_item} with the selected states.") 
                         else: 
                             pass # Proceed with available data 

                     df_plot_compare = df_compare.groupby(['Date', 'State'])['Price'].mean().reset_index() 
                     unit_for_display = WFP_UNITS_INFO.get(multi_state_food_item, "Unit N/A").replace("~", "") 
                     y_axis_label = f'Price (Naira / {unit_for_display})' if unit_for_display != "Unit N/A" else 'Price (Naira)' 

                     fig_multi_state = px.line( 
                         df_plot_compare, 
                         x='Date', 
                         y='Price', 
                         color='State', 
                         title=f'Price Trend for {multi_state_food_item} Across Selected States', 
                         labels={'Price': y_axis_label, 'Date': 'Date'}, 
                         hover_data={'Price': ':.2f'} 
                     ) 
                     fig_multi_state.update_layout(hovermode="x unified") 
                     st.plotly_chart(fig_multi_state, use_container_width=True) 
                 else: 
 ...
             # 6. Correlation Heatmap (Food Items vs. Food Price Index or other food items) 
             st.markdown("---") 
             st.markdown("#### ✅ 6. Correlation Heatmap") 
             st.markdown("Shows **how the prices of different food items (or food items against the Food Price Index) move together**. A high positive correlation means they tend to increase/decrease at the same time, while a high negative correlation means they move in opposite directions. The heatmap considers only the **last 12 months** of data.") 
 ...
                 
                 df_corr_data = st.session_state.df_full_merged[ 
                     (st.session_state.df_full_merged['Food_Item'].isin(st.session_state.capitalized_food_items)) & 
                     (st.session_state.df_full_merged['Date'] >= twelve_months_ago) 
                 ].copy() 

                 if not st.session_state.df_fpi.empty: 
                     df_fpi_corr = st.session_state.df_fpi[ 
                         (st.session_state.df_fpi['Date'] >= twelve_months_ago) 
                     ].copy() 
                     
                     # Rename 'Price' to 'Food_Price_Index_Value' in df_fpi_corr to avoid column name conflict after merge 
                     df_fpi_corr.rename(columns={'Price': 'Food_Price_Index_Value'}, inplace=True) 

                     # Merge df_corr_data with df_fpi_corr on 'Date' and 'State' 
                     df_pivot_combined = pd.merge( 
                         df_corr_data.pivot_table(index=['Date', 'State'], columns='Food_Item', values='Price'), 
                         df_fpi_corr.pivot_table(index=['Date', 'State'], columns='Food_Item', values='Food_Price_Index_Value'), 
                         on=['Date', 'State'], 
                         how='outer' 
                     ) 
                     # Ensure FPI column name is consistent after pivot 
                     if 'Food Price Index' in df_pivot_combined.columns: 
                         df_pivot_combined.rename(columns={'Food Price Index': 'Food_Price_Index'}, inplace=True) 

                     # Calculate correlation matrix 
                     if not df_pivot_combined.empty: 
                         corr_matrix = df_pivot_combined.corr() 
                         fig_corr = px.imshow( 
                             corr_matrix, 
                             text_auto=True, 
                             color_continuous_scale="RdBu", 
                             title=f'Correlation Heatmap of Food Items and FPI (Last 12 Months: {twelve_months_ago.strftime("%b %Y")} - {current_date.strftime("%b %Y")})' 
                         ) 
                         st.plotly_chart(fig_corr, use_container_width=True) 

                         # Smart Insights 
                         st.markdown("##### Smart Insights: Highest and Lowest Correlations") 
                         
 ...
                         lowest_corr_pairs = [] 
                         
                         # Iterate to find unique non-self correlations 
                         for i, ((item1, item2), corr_value) in enumerate(sorted_correlations.items()): 
                             if item1 != item2: 
                                 # To avoid duplicates (A-B vs B-A) 
                                 if f"{item2}-{item1}" not in [f"{p[0]}-{p[1]}" for p in highest_corr_pairs] and \ 
                                     f"{item2}-{item1}" not in [f"{p[0]}-{p[1]}" for p in lowest_corr_pairs]: 
                                     
                                     if len(highest_corr_pairs) < 1 and corr_value < 1.0: # Find top 1 highest (excluding perfect self-correlation) 
                                         highest_corr_pairs.append((item1, item2, corr_value)) 
                                     if len(lowest_corr_pairs) < 1 and corr_value > -1.0: # Find top 1 lowest (excluding perfect self-correlation) 
                                         lowest_corr_pairs.append((item1, item2, corr_value)) 

                         if highest_corr_pairs: 
                             item1, item2, corr_value = highest_corr_pairs[0] 
                             st.write(f"- **Highest positive correlation:** **{item1}** and **{item2}** with a coefficient of **{corr_value:.2f}**. This suggests their prices tend to move in the same direction.") 
                         if lowest_corr_pairs: 
                             item1, item2, corr_value = lowest_corr_pairs[0] 
                             st.write(f"- **Lowest (most negative) correlation:** **{item1}** and **{item2}** with a coefficient of **{corr_value:.2f}**. This indicates their prices tend to move in opposite directions.") 
                         else: 
                             st.info("Not enough data to compute meaningful correlations for smart insights.") 

                     else: 
                         st.info("Not enough data points in the last 12 months to compute a correlation heatmap.") 
                 else: 
                     # If FPI is empty but other data exists, just use food items 
                     st.warning("Food Price Index data not available for correlation. Computing correlation only for selected food items.") 
                     df_pivot = df_corr_data.pivot_table(index=['Date', 'State'], columns='Food_Item', values='Price') 
 ...
                             text_auto=True, 
                             color_continuous_scale="RdBu", 
                             title=f'Correlation Heatmap of Food Items (Last 12 Months: {twelve_months_ago.strftime("%b %Y")} - {current_date.strftime("%b %Y")})' 
                         ) 
                         st.plotly_chart(fig_corr, use_container_width=True) 
 ...
                                     f"{item2}-{item1}" not in [f"{p[0]}-{p[1]}" for p in lowest_corr_pairs]: 
                                     
                                     if len(highest_corr_pairs) < 1 and corr_value < 1.0: 
                                         highest_corr_pairs.append((item1, item2, corr_value)) 
                                     if len(lowest_corr_pairs) < 1 and corr_value > -1.0: 
                                         lowest_corr_pairs.append((item1, item2, corr_value)) 
 ...
                             st.info("Not enough data to compute meaningful correlations for smart insights.") 
                     else: 
                         st.info("Not enough data points in the last 12 months to compute a correlation heatmap for food items.") 
             else: 
                 st.info("No data loaded to compute correlations.") 

             # 7. Food Price Index Trend 
             st.markdown("---") 
             st.markdown("#### ✅ 7. Food Price Index (FPI) Trend") 
             st.markdown("Visualizes the **overall trend of the Food Price Index**, providing a high-level view of food inflation across the country. Lower values indicate lower food prices across the board (on average), and vice-versa for higher values.") 

             if not st.session_state.df_fpi.empty: 
                 fpi_states = ['Nationwide Average (FPI)'] + sorted(st.session_state.df_fpi['State'].unique().tolist()) 
                 selected_fpi_state = st.selectbox( 
                     "Select State for FPI Trend:", 
                     fpi_states, 
                     key="fpi_state_select" 
                 ) 
                 
                 if selected_fpi_state == 'Nationwide Average (FPI)': 
                     df_fpi_plot = st.session_state.df_fpi.groupby('Date')['Price'].mean().reset_index() 
                     title_fpi = 'National Average Food Price Index Trend Over Time' 
                 else: 
                     df_fpi_plot = st.session_state.df_fpi[st.session_state.df_fpi['State'] == selected_fpi_state].groupby('Date')['Price'].mean().reset_index() 
                     title_fpi = f'Food Price Index Trend for {selected_fpi_state} Over Time' 

                 if not df_fpi_plot.empty: 
                     fig_fpi = px.line( 
                         df_fpi_plot, 
                         x='Date', 
                         y='Price', 
 ...
                 else: 
                     st.info(f"No Food Price Index data available for {selected_fpi_state}.") 
             else: 
                 st.info("No Food Price Index data available.") 
             

             # 8. Time Series Forecasting (ARIMA) 
             st.markdown("---") 
             st.markdown("#### ✅ 8. Price Forecasting (ARIMA Model)") 
             st.markdown("Leverages pre-trained ARIMA models to **forecast future food prices**. This helps anticipate price changes and plan accordingly.") 

             forecast_food_item = st.selectbox( 
                 "Select Food Item to Forecast:", 
                 st.session_state.capitalized_food_items, 
                 key="forecast_food_select" 
             ) 
             forecast_months = st.slider("Number of Months to Forecast:", min_value=1, max_value=12, value=3, key="forecast_months_slider") 

             if forecast_food_item: 
                 # Prepare the time series data for the model 
                 df_food_item_for_forecast = st.session_state.df_full_merged[ 
                     st.session_state.df_full_merged['Food_Item'] == forecast_food_item 
                 ].copy() 

                 if not df_food_item_for_forecast.empty: 
                     ts_log_series = prepare_time_series_for_arima(df_food_item_for_forecast, forecast_food_item) 

                     if not ts_log_series.empty: 
                         # Generate a hash of the current time series data to enable caching 
                         ts_hash = pd.util.hash_pandas_object(ts_log_series).sum() 
                         
                         forecast_series, conf_int_df = load_and_forecast_arima_model(forecast_food_item.lower(), ts_hash, forecast_months) 

                         if not forecast_series.empty: 
                             # Combine historical and forecast data for plotting 
                             historical_prices = df_food_item_for_forecast.groupby('Date')['Price'].mean() 
                             combined_series = pd.concat([historical_prices, forecast_series]) 

                             fig_forecast = px.line( 
                                 combined_series.reset_index(), 
                                 x='index', 
                                 y='Price', 
                                 title=f'Price Forecast for {forecast_food_item}', 
                                 labels={'index': 'Date', 'Price': f'Price (Naira / {WFP_UNITS_INFO.get(forecast_food_item, "Unit N/A").replace("~", "")})'} 
                             ) 
 ...
                                 go.Scatter( 
                                     x=conf_int_df.index.tolist() + conf_int_df.index[::-1].tolist(), 
                                     y=conf_int_df['upper'].tolist() + conf_int_df['lower'][::-1].tolist(), 
                                     fill='toself', 
 ...
                                 ) 
                             ]) 
                             fig_forecast.update_layout(showlegend=True, hovermode="x unified") 
                             st.plotly_chart(fig_forecast, use_container_width=True) 

                             st.markdown("##### Forecasted Prices:") 
                             forecast_table_data = forecast_series.reset_index() 
                             forecast_table_data.columns = ['Date', 'Forecasted Price'] 
                             forecast_table_data['Date'] = forecast_table_data['Date'].dt.strftime('%Y-%m') 
                             st.dataframe(forecast_table_data, use_container_width=True) 

                         else: 
                             st.info(f"Could not generate forecast for {forecast_food_item}.") 
                     else: 
                         st.info(f"Not enough data to prepare time series for {forecast_food_item}.") 
                 else: 
                     st.info(f"No data available for {forecast_food_item} to perform forecasting.") 
             else: 
                 st.info("Please select a food item for forecasting.") 
     else: 
         st.info("Please click 'Load All Data' in the sidebar to begin.")
