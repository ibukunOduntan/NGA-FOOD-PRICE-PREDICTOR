...
def prepare_time_series_with_exog(df, state_name, food_item):
    """
    Preparz the time series (Price) and exogenous variables (foodYearOn, rain)
...
# Note: With no persistent DB, models are trained on demand.
...
def forecast_food_prices_sarimax(ts, exog, food_item, state_name, forecast_steps, df_full_merged):
    ...
    """
    # Consolidated spinner for model training
    with st.spinner(f"Fetching data and training model for {food_item} in {state_name}... please wait"):

        # No joblib loading/saving as we're not using a persistent file system for models
        # The model will be re-trained or loaded from Streamlit's cache_resource.
...
years_back_explorer = st.sidebar.slider(
    "No. of years:", min_value=1, max_value=10, value=5, key="years_back_explorer"
)

# Initialize session state variables using .get() for robustness
st.session_state.df_full_merged = st.session_state.get('df_full_merged', None)
st.session_state.df_food_prices_for_explorer = st.session_state.get('df_food_prices_for_explorer', None)
...
with tab1:
    ...
    if st.session_state.df_food_prices_for_explorer is not None and not st.session_state.df_food_prices_for_explorer.empty:
        ...
        if food_data_explorer_filtered.empty:
            ...
            st.info("No data available for display. Please set your filter options in the side bar.")
    ...
    with tab2:
        ...
        st.markdown("""Data sources:
                    Historical rices: World Bank API,
                ...
                if ts.empty or exog.empty:
                    ...
                    with st.spinner("Generating forecast using SARIMAX..."):
                        forecast_series = forecast_food_prices_sarimax(
                            ...
                            forecast_months,
                            st.session_state.df_full_merged
                        )
